layui.define(['element', 'jquery', 'form', 'laytpl'], function (exports) {
    ...
    Class.prototype.iniLabelSelectorWindow = function () {
        let that = this,
            ...
            index = that.index,
            elem = config.elem,
            windowConfig = config?.window ?? { windowStyle: "", title: { text: "请选择合适的标签项", style: "" } };
    ...
    labelSelector.render = function (options) {
        ...
        if (inst.index === 1)
            console.log("欢迎使用Hg科技的labelSelector组件，version:" + labelSelector.version + ",期待您的建议！");
    }

    exports('labelSelector', labelSelector);
}).link('./layui-exts/labelSelector/css/labelSelector.css');